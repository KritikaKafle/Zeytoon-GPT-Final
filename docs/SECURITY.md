...
3. **In-Depth Understanding**: Reports must reflect a clear understanding of the codebase, how Zeytoon GPT is used and provide specific details about the vulnerability, including the affected components and potential impacts.
...
6. **Default Configuration Testing**: All vulnerability reports must be tested and reproducible using Zeytoon GPT's out-of-the-box default configuration. Claims of vulnerabilities that only manifest with explicitly weakened security settings may be discarded, unless they are covered by the following exception:
...
7. **Threat Model Understanding Required**: Reports must demonstrate understanding of Zeytoon GPT's self-hosted, authenticated, extensible, role-based access control architecture. Comparing Zeytoon GPT to services with fundamentally different security models without acknowledging the architectural differences may result in report rejection.
...
>
> - If we suspect you used AI (but you did not disclose it to us), we will be asking thorough follow-up questions to validate your understanding of the reported vulnerability and Zeytoon GPT itself.
...
> - had a clear lack of understanding of Zeytoon GPT
...
Due to the volume of incoming vulnerability reports, issues, discussions, pull requests, and general project maintenance — lately compounded by a large number of invalid AI-generated reports (see [AI report transparency](#ai-report-transparency)) — our capacity to respond is limited. Zeytoon GPT is a community-driven project maintained by a small team, and security reports are handled alongside all other project responsibilities.
...
Premature disclosure undermines the security of all Zeytoon GPT users and **violates the trust** inherent in the responsible disclosure process. **Reporters who publicly disclose vulnerability details before official publication <ins>may be permanently banned from future reporting.</ins>**
