...
async def send_post_request(
    ...
    try:
        ...
        if r.ok is False:
            try:
                ...
                raise HTTPException(
                    ...
                    detail=f'Zeytoon GPT: Server Connection Error',
                )
        ...
        raise HTTPException(
            ...
            detail=detail if e else 'Zeytoon GPT: Server Connection Error',
        )
    finally:
        if not streaming:
            await cleanup_response(r, session)
...
async def verify_connection(form_data: ConnectionVerificationForm, user=Depends(get_admin_user)):
    ...
    async with aiohttp.ClientSession(
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail='Zeytoon GPT: Server Connection Error')
        except Exception as e:
    ...
    if url_idx is None:
        ...
        try:
            ...
            raise HTTPException(
                ...
                detail=detail if detail else 'Zeytoon GPT: Server Connection Error',
            )
...
async def get_openai_models(
    ...
    if url_idx is None:
        ...
        try:
            ...
            log.exception(e)
            error_detail = 'Zeytoon GPT: Server Connection Error'
            if r is not None:
                try:
                    res = r.json()
                    if 'error' in res:
